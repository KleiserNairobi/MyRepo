...
export default function Banco() {
  ...
  const colunasTabela = [    
    ...
    { title: 'Código', field: 'codigo', cellStyle: {width: '30px'} },
    { title: 'Nome', field: 'nome', width: 3000 }
  ];
  ...
  useEffect(() => {
    if (confirmaExcluir && id > 0) {
      ...
      async function excluiRegistro() {
        try {
          const resposta = await service.exclui(`/bancos/${id}`);
  ...
  return (
    <div>
      <CabecalhoForm
        titulo="Bancos"
        subtitulo="Listagem de todas os bancos"
