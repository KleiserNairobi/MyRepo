...
export default function Habilitacao() {
  ...
  const colunasTabela = [
    ...
    { title: 'Membro', field: 'pessoa.nome', cellStyle: {width: '350px', fontSize: 13} },
    { title: 'Registro', field: 'registro', cellStyle: {width: '100px', fontSize: 13} },
    { title: 'Local Expedição', field: 'localExpedicao', cellStyle: {width: '200px', fontSize: 13} },
    { title: 'Emissão', field: 'dataEmissao', type: 'date', cellStyle: {width: '100px', fontSize: 13} },
    { title: 'Validade', field: 'validade', type: 'date', width: 3000, cellStyle: {fontSize: 13} },
  ];
  ...
  useEffect(() => {
    if (excluir && id > 0) {
      async function buscaDado() {
        const resposta = await service.obtem(`/habilitacoes/${id}?campos=registro`);
        if (resposta.data) {
          setTexto(`REG.: ${id} - ${resposta.data.registro}`);
        }
  ...
  useEffect(() => {
    if (confirmaExcluir && id > 0) {
      ...
      async function excluiRegistro() {
        try {
          const resposta = await service.exclui(`/habilitacoes/${id}`);
  ...
  useEffect(() => {
    if (alterar) {
      history.push('/habilitacoes-form');
    }
  }, [alterar, history])

  // Effect para carregamento das habilitacoes
  useEffect(() => {
    if (carregar) {
      setCarregando(true);
      service.obtem('/habilitacoes')
        .then(response => {
  ...
  return (
    <div>
      <CabecalhoForm
        titulo="Habilitações"
        subtitulo="Listagem de todas as habilitações"
        linkPagina="/habilitacoes-form"
