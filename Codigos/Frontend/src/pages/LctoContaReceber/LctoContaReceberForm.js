...
import { ArrowBack, Archive, YoutubeSearchedFor, DeviceHub } from '@material-ui/icons';
...
import * as service from '../../services/LctoContaReceberService';
import { useAlerta } from '../../contexts/AlertaCtx';
import { useGeral } from '../../contexts/GeralCtx';
import PageCss from '../PagesCss';
import schema from './LctoContaReceberSch';
...
export default function LctoContaReceberForm() {
  ...
  const colunasTabelaParcelas = [    
    ...
    { title: 'Data Recebimento', field: 'dataRecebimento', type: 'date' },
    { title: 'Valor Recebido', field: 'valorRecebimento', type: 'currency', 
      currencySetting: {
        locale: 'pt-BR', currencyCode:'BRL', 
        minimumFractionDigits: 2, maximumFractionDigits: 2
      },
  ...
  useEffect(() => {
    if (alterar) {
      async function buscaDado() {
        try {
          const resposta = await service.obtem(`/contas-receber/${id}`);
          if (resposta.data) {
            ...
            setValue('ValorReceber', resposta.data.ValorReceber * 100);
            ...
            setValorReceber(resposta.data.valorReceber * 100);

            service.obtem(`/parcelas-conta-receber/${id}`)
            .then(resposta => {
              var lista = resposta.data.map(function (item) {
                return {
                  id: item.id,
  ...
  function submitForm(data) {
    if (alterar & id > 0) {
      async function alteraDado() {
        try {
          const resposta = await service.altera(`/contas-receber/${id}`, data);
          if (resposta) {
            setConteudo({
              tipo: TIPO_SUCESSO,
      ...
      async function insereDado() {
        try {
          const resposta = await service.insere('/contas-receber', data);
          if (resposta) {
            setConteudo({
              tipo: TIPO_SUCESSO,
  ...
  return (
    <div>
      <CabecalhoForm
        ...
        subtitulo="Adição de nova conta a receber"
        linkPagina="/lctoContaReceber"
        icone={<ArrowBack />}
      ...
      <form onSubmit={handleSubmit(submitForm)}>
        <Card variant="outlined">
          <CardContent>
            <Card>
              <CardHeader
                avatar={<Avatar className={css.avatar}><Archive /></Avatar>}
                title="Conta a Receber"
                subheader="Informe os dados da conta a receber"
                ...
                <Grid container direction="row" spacing={2}>
                  ...
                  <Grid item xs={12} sm={6} md={3} >
                    <TextField
                      ...
                      inputRef={register}
                      value={valorReceber}
                      onChange={(ev)=>setValorReceber(ev.target.value) }
                      InputLabelProps={{ shrink: true }}
                      InputProps={{inputComponent: MyTFMoneyNumberFormat } }
                      disabled
                    />
                ...
                {exibeParcelas ?
                  <Card>
                    <CardHeader
                      ...
                      title="Parcelas da Conta a Receber"
                      subheader="Listagem das parcelas da conta a receber"
                      className={css.cartaoTitulo}
                    />
                    <MyTable
