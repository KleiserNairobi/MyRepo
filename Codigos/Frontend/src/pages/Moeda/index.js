...
export default function Moeda() {
  ...
  useEffect(() => {
    if (confirmaExcluir && id > 0) {
      ...
      async function excluiRegistro() {
        try {
          const resposta = await service.exclui(`/moedas/${id}`);
  ...
  useEffect(() => {
    if (alterar) {
      history.push('/moedas-form');
    }
  }, [alterar, history])

  // Effect para carregamento das moedas
  useEffect(() => {
    if (carregar) {
      setCarregando(true);
  ...
  return (
    <div>
      <CabecalhoForm
        titulo="Moedas"
        subtitulo="Listagem de todas as moedas"
        linkPagina="/moedas-form"
      ...
      <Card className={css.root}>
        <MyTable
          ...
          exportar={true}
          titulo="Listagem de todas as moedas"
        />
      </Card>
      <ModalExclusao
