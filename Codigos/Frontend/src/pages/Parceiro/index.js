...
export default function Parceiro() {
  ...
  useEffect(() => {    
    if (excluir && id > 0) {
      console.log('chamou modal de exclusão')
  ...
  useEffect(() => {
    if (confirmaExcluir && id > 0) {
      console.log('preparando url de exclusão')
      setCarregando(true);
      async function excluiRegistro() {
        try {
          ...
          if (resposta.status === 204) {
            setConteudo({
              tipo: TIPO_SUCESSO,
              descricao: 'Registro inativado com sucesso!',
              exibir: true
            });
            setId(-1);
  ...
  useEffect(() => {
    if (alterar) {
      history.push('/parceiros-form');
    }
  }, [alterar, history])

  // Effect para carregamento dos parceiros
  useEffect(() => {
    if (carregar) {
      setCarregando(true);
      service.obtem('/pessoas/parceiros')
        .then(response => {
  ...
  return (
    <div>
      <CabecalhoForm
        titulo="Parceiros"
        subtitulo="Listagem de todos os parceiros"
        linkPagina="/parceiros-form"
