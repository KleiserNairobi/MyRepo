...
export default function Veiculo() {
  ...
  const colunasTabela = [
    ...
    { title: 'Tipo', field: 'tipo', cellStyle: {width: '40px', fontSize: 13} },
    { title: 'Modelo', field: 'modelo', cellStyle: {width: '250px', fontSize: 13} },
    { title: 'Renavan', field: 'renavan', cellStyle: {width: '100px', fontSize: 13} },
    { title: 'Placa', field: 'placa', cellStyle: {width: '100px', fontSize: 13} },
  ...
  useEffect(() => {
    if (excluir && id > 0) {
      async function buscaDado() {
        const resposta = await service.obtem(`/veiculos/${id}?campos=modelo`);
        if (resposta.data) {
          setTexto(`REG.: ${id} - ${resposta.data.modelo}`);
        }
  ...
  useEffect(() => {
    if (confirmaExcluir && id > 0) {
      ...
      async function excluiRegistro() {
        try {
          const resposta = await service.exclui(`/veiculos/${id}`);
  ...
  useEffect(() => {
    if (alterar) {
      history.push('/veiculos-form');
    }
  }, [alterar, history])

  // Effect para carregamento dos veículos
  useEffect(() => {
    if (carregar) {
      setCarregando(true);
      service.obtem('/veiculos')
      .then(response => {
  ...
  return (
    <div>
      <CabecalhoForm
        titulo="Veículos"
        subtitulo="Listagem de todas os veículos"
        linkPagina="/veiculos-form"
