...
const sourceBackground = require('../../assets/images/Cliente_320L.png');
...
class LoginPage extends ValidationComponent {
        ...
        .then(res => {
          if (res.data) {
            if (res.data.access_token) {
              ...
              API({ token: res.data.access_token }).get(`usuarios/${idUser}`)
                .then(res2 => {
                  if (res2 && res2.data) {
                    try {
                      ...
                      } else if (res2.data.pessoa && res2.data.pessoa.cliente === false) {
                        erro = 'Cadastro não habilitado para Modo Cliente';
                      }
  ...
  validateInput = (input, valueInput) => {
    switch (input) {
      ...
      case 'telefone': 
        valueInput = valueInput.trim();
        this.setState({ telefone: valueInput }); break;
    ...
    return (
      <StyleProvider style={getTheme(material)}>
        <KeyboardAvoidingView style={Styles.body}>
          <ImageBackground source={sourceBackground} style={{ flex: 1 }} imageStyle={{ height: '36%', resizeMode: 'contain', top: undefined }}>
            <Brand />
            <View style={Styles.contentContainer}>
              <View style={Styles.formContainer}>
                ...
                <View style={[Styles.horizontalContainer, { width: '100%' }]}>
                  <Text style={[Styles.font12, { width: '50%' }]}>ESQUECEU SUA SENHA?</Text>
                  <TouchableOpacity 
